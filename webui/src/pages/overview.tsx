...
export default function OverviewPage() {
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-semibold">Scanner overview</h1><p className="mt-1 text-sm text-muted-foreground">Fixed-order scanning with report-only AI.</p></div><Button asChild><Link to="/scans/new"><Plus className="h-4 w-4" /> New scan</Link></Button></header>
    ...
    <Card><CardHeader><CardTitle>Scanner health</CardTitle></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{scannerHealth.map((s) => <div key={s.name} className="rounded-md border p-3"><p className="font-medium capitalize">{s.name}</p><p className={`mt-2 text-xs ${s.available ? "text-emerald-400" : "text-red-400"}`}>{s.available ? "available" : "unavailable"}</p><p className="mt-1 truncate text-[10px] text-muted-foreground">{s.path || (s.endpoint_configured ? "service configured" : "service not configured")}</p></div>)}</div></CardContent></Card>
