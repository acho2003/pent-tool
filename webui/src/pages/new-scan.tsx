...
import { SCANNER_ORDER } from "@/types/api";
...
// What each scanner needs from the submitted target, so deselecting one is an
// informed choice rather than a guess.
const SCANNER_SCOPE: Record<string, string> = {
  nuclei: "Template scan of the submitted host or URL",
  zap: "Spider and active scan of an HTTP/HTTPS target",
  openvas: "Greenbone network scan of the derived host",
  trivy: "Artifact scan — needs an artifact below",
  vuls: "Host CVE audit — needs an SSH alias below",
};
...
export default function NewScanPage() {
  ...
  const [scanners, setScanners] = useState<string[]>([...SCANNER_ORDER]);
  const health = useQuery({ queryKey: ["scanner-status"], queryFn: api.scannerStatus, refetchInterval: 30000 });
  const availability = useMemo(() => {
    const map = new Map<string, boolean>();
    for (const s of health.data?.scanners ?? []) map.set(s.name, s.available);
    return map;
  }, [health.data]);
  ...
  function toggleScanner(name: string) {
    setScanners((prev) => (prev.includes(name) ? prev.filter((s) => s !== name) : [...prev, name]));
  }
  ...
  async function submit(saveOnly: boolean) {
    ...
    try {
      const res = await start.mutateAsync({
        ...
        // All five selected is the default; send nothing so the scan is not
        // pinned to today's pipeline membership.
        scanners: scanners.length === SCANNER_ORDER.length ? undefined : scanners,
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">Nuclei, ZAP, OpenVAS, Trivy, and Vuls run in fixed order. Report AI runs only after scanning is complete.</p>
    </div>
    <form onSubmit={onSubmit} className="space-y-5">
      <Card><CardHeader><CardTitle>Target and mode</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        <div className="space-y-2"><Label htmlFor="targets">Hosts or URLs</Label><Textarea id="targets" value={targetsText} onChange={(e) => setTargetsText(e.target.value)} placeholder={"https://example.com\napi.example.com"} rows={4} /><p className="text-xs text-muted-foreground">One per line. Nuclei receives the submitted value; ZAP and OpenVAS derive URL/host inputs deterministically.</p></div>
      ...
      <Card><CardHeader><CardTitle>Scanners</CardTitle></CardHeader><CardContent className="space-y-3">
        <div className="grid gap-2 sm:grid-cols-2">{SCANNER_ORDER.map((name) => {
          const available = availability.get(name);
          return <label key={name} className="flex items-start gap-2 rounded-lg border p-3 text-sm">
            <input type="checkbox" checked={scanners.includes(name)} onChange={() => toggleScanner(name)} className="mt-0.5 h-3.5 w-3.5 rounded border-border" />
            <span className="min-w-0">
              <span className="flex items-center gap-2"><span className="font-medium capitalize">{name}</span>{available === false && <span className="text-xs text-red-400">unavailable</span>}{available === true && <span className="text-xs text-emerald-400">ready</span>}</span>
              <span className="mt-0.5 block text-xs text-muted-foreground">{SCANNER_SCOPE[name]}</span>
            </span>
          </label>;
        })}</div>
        <p className="text-xs text-muted-foreground">Deselected scanners are recorded as <span className="font-mono">skipped</span> in the scan, so the report still shows what was not attempted. Selected scanners that cannot handle the target stay <span className="font-mono">not applicable</span>.</p>
      </CardContent></Card>
