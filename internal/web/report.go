...
func (s *Server) generateReport(scan *ScanRecord) (string, error) {
	...
	if len(scan.Vulns) > 0 {
		...
		for idx, v := range scan.Vulns {
			...
			// Schema-v2 findings are always labelled scanner-reported. The report
			// generator does not claim independent exploitation or verification.
	...
	disclaimer := `This assessment was conducted by Xalgorix using a deterministic sequence of security scanners. Findings are scanner-reported and derived from preserved native scanner artifacts. Report AI may explain or deduplicate source records but does not control scanning, exploit targets, independently verify findings, or introduce untraceable findings.
...
* False Positives: Scanner results may require manual validation. Review native evidence and validate critical and high-severity findings before taking remediation actions.
...
Generated by Xalgorix - Deterministic Scanner Pipeline
